...
const MUTATIONS = [
  ...
  ['M95', 'Gen III stops halving spread damage and takes the modern three quarters',
    "  return g===3?0.5:0.75;", '  return 0.75;', 1, 'test-spread-moves.js'],
  ['M96', 'A spread reduction is claimed in Gen I and II, where double battles did not exist',
    '  if(g<3)return null;      // double battles did not exist yet', '  if(false)return null;',
    1, 'test-spread-moves.js'],
  ['M97', 'Earthquake stops warning that it hits your own partner',
    "const SPREAD_TARGETS={allAdjacentFoes:'both opponents',allAdjacent:'both opponents and your partner'};",
    "const SPREAD_TARGETS={allAdjacentFoes:'both opponents',allAdjacent:'both opponents'};",
    1, 'test-spread-moves.js'],
  ['M98', 'A status spread move is given a damage multiplier it has no damage to apply',
    "if(m.category==='Status')return {target:m.target,hits:SPREAD_TARGETS[m.target],mult:null};",
    '', 1, 'test-spread-moves.js'],
  ['M99', 'The badge caches Gen IX instead of re-reading the generation on screen',
    'mult:spreadMultForGen(getDataGenNum())', 'mult:spreadMultForGen(9)', 1, 'test-spread-moves.js'],
  ['M100', 'A move with no other tags loses its spread badge, hiding it on Dazzling Gleam',
    "  if(!flags.length)return spread?'<div class=\"mv-tags\">'+spread+'</div>':'';",
    "  if(!flags.length)return '';", 1, 'test-spread-moves.js'],
  ['M101', 'The learnset table stops marking spread moves',
    'renderSpreadTag(m.name,true)', '', 2, 'test-spread-moves.js'],
  ['M102', 'An unrecognised target is treated as a spread target',
    'if(!m||!m.target||!SPREAD_TARGETS[m.target])return null;', 'if(!m||!m.target)return null;',
    1, 'test-spread-moves.js'],

  ['M103', 'The bare multiplier returns to the chip face, so a move claims a boost it does not have',
    "+'\">'+i.label+'</span>';",
    "+'\">'+i.label+(boost?'<b>x</b>':'')+'</span>';", 1, 'test-move-tags.js'],
  ['M104', 'A boost goes back to a name beside a number, with the verb left to the reader',
    "if(r.kind==='boost')out.push({t:who+' boosts it'+m,boost:true});",
    "if(r.kind==='boost')out.push({t:who+m,boost:true});", 1, 'test-move-tags.js'],

  /* 5.44 - the pill system: one solid component, colour as the only carrier of meaning. */
  ['M105', "A type pill goes back to a translucent wash of its colour",
    ".type-badge{--tc:#888;--tc-ink:#fff;background:var(--tc);",
    ".type-badge{--tc:#888;--tc-ink:#fff;background:color-mix(in srgb,var(--tc) 15%,transparent);", 1, 'test-pill-system.js'],
  ['M106', "The derived ink is replaced by one fixed colour, so Electric goes white-on-yellow",
    "return (TC_INK[hex]=ratio(L,1)>=ratio(L,0.0058)?'#ffffff':'#12121a');",
    "return (TC_INK[hex]='#ffffff');", 1, 'test-pill-system.js'],
  ['M107', "A pill fill and its ink stop being written together, so the ink is inherited",
    "return '--tc:'+hex+';--tc-ink:'+tcInk(hex);",
    "return '--tc:'+hex;", 1, 'test-pill-system.js'],
  ['M108', "Colourblind mode loses its pill inks, leaving white text on its brighter fills",
    "--eff-zero:#9a9a96;--eff-zero-ink:#15151f;--pill-up-ink:#12121a;--pill-dn-ink:#12121a}",
    "--eff-zero:#9a9a96;--eff-zero-ink:#15151f}", 1, 'test-pill-system.js'],
  ['M109', "An empty engine flags object short-circuits the build-time table again",
    "          if(fromEngine.length)return fromEngine;",
    "          return fromEngine;", 1, 'test-move-tags.js'],
  ...
  /* 5.47 - Regulation M-C, and Champions' own move data. */
  ['M118', "Champions move values stop applying, so Protect shows 10 PP again",
    "  return o?o[field]:undefined;\n}",
    "  return undefined;\n}", 1, 'test-champions-mc.js'],
  ['M119', "A question about another generation, asked from Champions, gets Champions' answer",
    "  if(genNum!==undefined&&genNum!==getDataGenNum())return undefined;\n",
    "", 1, 'test-champions-mc.js'],
  ['M120', "The calculator's flag override replaces instead of merging, stripping contact from Dragon Claw",
    "    const base=Object.assign({},((gen.moves&&gen.moves.get(id))||{}).flags||{});",
    "    const base={};", 1, 'test-champions-mc.js'],
  ['M121', "Changing regulation stops swapping the item list, so M-B shows M-C's stones",
    "  CHAMPIONS_ITEMS=new Set(CHAMPIONS_ITEMS_BY_REG[r.key].split(','));\n",
    "", 1, 'test-champions-mc.js'],
  ['M122', "The Regulation Changes page reports the source disagreements as things M-C changed",
    "if(!inFrom.has(k)&&both.has(v.num)&&!srcDiff.has(k))gained.push(v)",
    "if(!inFrom.has(k)&&both.has(v.num))gained.push(v)", 1, 'test-champions-mc.js'],
  ['M123', "The app opens on Regulation M-B again",
    "  {key:'reg-mc', short:'m-c', label:'Regulation M-C', ids:()=>CHAMPIONS_IDS_MC},\n",
    "", 1, 'test-champions-mc.js'],
  ['M124', "Champions' extra move flags are ignored, so Dragon Claw loses Slicing",
    "  add.forEach(function(f){if(MOVE_TAG_NOTE[f]&&out.indexOf(f)<0)out.push(f)});\n",
    "", 1, 'test-champions-mc.js'],
  ['M125', "Champions' type changes are ignored, so Snap Trap is Grass again",
    "  if(champType)return champType.toLowerCase();\n",
    "", 1, 'test-champions-mc.js'],

  /* 5.49 - speed tiers from embedded stats, illegal forms, card pills, sort badge, held-item groups. */
  ['M126', "Illegal forms pass formAllowed again, so costume Pikachu reaches the calculator",
    "  if(isChampionsMode&&typeof champFormIllegal==='function'&&champFormIllegal(name))return false;\n",
    "", 1, 'test-champions-mc.js'],
  ['M127', "The embedded stats stop deferring to a fetched record, so two sources can disagree on screen",
    "  if(dc[p.id])return getStatsForGen(dc[p.id]);\n  const b=champBaseStats(p);",
    "  const b=champBaseStats(p);", 1, 'test-champions-mc.js'],
  ['M128', "Speed Tiers goes back to fetching every Pokemon's full record",
    "ensureRosterLoaded(renderSpeedTiers,roster.filter(function(p){return !rosterStatList(p)}).map(function(p){return p.id}));",
    "ensureRosterLoaded(renderSpeedTiers,roster.map(function(p){return p.id}));", 1, 'test-champions-mc.js'],
  ['M129', "The Paldean Tauros stop resolving, because PokeAPI calls them breeds",
    ".replace(/-breed$/,'')",
    "", 1, 'test-champions-mc.js'],
  ['M130', "Dex cards wrap their type pills again, so Medicham's stack",
    ".card-types{display:flex;gap:4px;justify-content:center;flex-wrap:nowrap}",
    ".card-types{display:flex;gap:4px;justify-content:center;flex-wrap:wrap}", 1, 'test-pill-system.js'],
  ['M131', "The shrink threshold drops below the widest pair in a fallback font",
    "@container (max-width:159px){",
    "@container (max-width:149px){", 1, 'test-pill-system.js'],
  ['M132', "The sort badge goes back to reading HP160",
    ".card-sortstat{display:flex;align-items:baseline;gap:6px;margin-bottom:2px}",
    ".card-sortstat{margin-bottom:2px}", 1, 'test-pill-system.js'],
  ['M133', "The Items tab ignores the held-item groups and shows one bucket again",
    "const cat=(catId===12&&HELD_ITEM_GROUP[item.name])||catName;",
    "const cat=catName;", 1, 'test-item-groups.js'],
  ['M134', "Binding Band is filed as an Extender",
    "'Extenders':['damp-rock','heat-rock','icy-rock','smooth-rock','light-clay','terrain-extender','grip-claw'],",
    "'Extenders':['damp-rock','heat-rock','icy-rock','smooth-rock','light-clay','terrain-extender','grip-claw','binding-band'],", 1, 'test-item-groups.js'],
  ['M135', "Fairy Feather goes back among the held items",
    "    'Type Enhancement':['fairy-feather'],\n",
    "", 1, 'test-item-groups.js'],
  // 5.50: snapshots for the list views, the move records, and the derived priority table.
  ['M136', "A snapshot record outranks a full one that has already downloaded",
    "function dexRec(id){return dc[id]||dl[id]}", "function dexRec(id){return dl[id]||dc[id]}", 1, 'test-dex-search.js'],
  ['M137', "The snapshot writes into dc, where a partial record passes for a full one",
    "rows.forEach(function(row){dl[row[0]]=liteRecord(row)});",
    "rows.forEach(function(row){dl[row[0]]=dc[row[0]]=liteRecord(row)});", 1, 'test-fast-load.js'],
  ['M138', "Dex cards ignore the snapshot and wait for full records again",
    "function cardHTML(e){\n  const d=dexRec(e.id);", "function cardHTML(e){\n  const d=dc[e.id];", 1, 'test-fast-load.js'],
  ['M139', "The type filter only sees Pokemon whose full record has downloaded",
    "if(t)f=f.filter(p=>{const d=dexRec(p.id);", "if(t)f=f.filter(p=>{const d=dc[p.id];", 1, 'test-fast-load.js'],
  ['M140', "Startup ignores the snapshot and waits on PokeAPI's lists",
    "const _haveIndex=await loadDexIndex();", "const _haveIndex=false;", 1, 'test-fast-load.js'],
  ['M141', "Ability holders lose their id, so every holder filter drops them",
    "url:'https://pokeapi.co/api/v2/pokemon/'+h[1]+'/'", "url:'https://pokeapi.co/api/v2/pokemon/'", 1, 'test-fast-load.js'],
  ['M142', "Item categories are filed in the wrong order, so a shared item lands under the later one",
    "HELD_ITEM_CATEGORIES.forEach(function(catId,ci){", "HELD_ITEM_CATEGORIES.slice().reverse().forEach(function(catId,ci){", 1, 'test-fast-load.js'],
  ['M143', "Move records stop coming from the snapshot",
    "  if(idx&&idx[name])return expandMoveRow(idx[name]);\n", "", 1, 'test-fast-load.js'],
  ['M144', "A move expanded from the snapshot loses its history",
    "past_values:r[12].map(", "past_values:[].map(", 1, 'test-fast-load.js'],
  ['M145', "The priority table reads the next generation's value",
    "let v=inChamp?r[4]:r[3][genNum-1];", "let v=inChamp?r[4]:r[3][genNum];", 1, 'test-priority.js'],
  ['M146', "Champions lists priority moves the regulation does not allow",
    "if(inChamp&&!CHAMPIONS_MOVES.has(slug.replace(/-/g,'')))return;", "", 1, 'test-priority.js'],
  ['M147', "Grassy Glide disappears, because its priority is conditional",
    "if(note){const m=note.match(/^([+-]\\d+)/);if(m&&v===0)v=parseInt(m[1],10)}", "", 1, 'test-priority.js'],
  ['M148', "Priority moves lose their hover description",
    "onmouseenter=\"showMoveTip(event,\\''+m.slug+'\\')\" ", "", 1, 'test-priority.js'],
  ['M149', "The past-ability index is never rebuilt, so it goes stale",
    "if(!c[genNum]||c[genNum].n!==n){", "if(!c[genNum]){", 1, 'test-past-abilities.js'],
  ['M150', "The colourblind toggle comes back",
    "<button class=\"theme-toggle\" onclick=\"toggleTheme()\"", "<button id=\"cvd-btn\"></button><button class=\"theme-toggle\" onclick=\"toggleTheme()\"", 1, 'test-viz-palette.js'],
  // 5.51: PP and power per generation and game, Champions PP, Champions move effects.
  ['M151', "Champions shows the defined PP again, so Protect reads 5 where the game shows 8",
    "return inChamp?champInGamePP(md,v):v;", "return v;", 1, 'test-champion-moves.js'],
  ['M152', "Champions PP is no longer capped at 20 before converting",
    "return (Math.min(pp,CHAMP_PP.cap)/5+1)*4;", "return (pp/5+1)*4;", 1, 'test-champion-moves.js'],
  ['M153', "Sketch's PP gets converted like any other move's",
    "if(CHAMP_PP.noBoosts.indexOf(id)>=0)return pp;", "", 1, 'test-champion-moves.js'],
  ['M154', "Let's Go loses its own values and shows Sun/Moon's",
    "if(gm&&gm[field]!==undefined)return gm[field];", "", 1, 'test-champion-moves.js'],
  ['M155', "A game's values apply in every generation, not only its own",
    "&&VG_GEN[dataGen]===g){", "){", 1, 'test-champion-moves.js'],
  ['M156', "PP goes back to PokeAPI's, so Recover says 5 in Generation IV",
    "    const fix=moveValueFix(md,'pp',g);\n    if(fix!==undefined)v=fix;", "    const fix=undefined;\n    if(fix!==undefined)v=fix;", 1, 'test-champion-moves.js'],
  ['M157', "Power goes back to PokeAPI's, so Sun/Moon's Solar Beam says 200",
    "const fix=moveValueFix(md,'power',g);if(fix!==undefined)return fix;", "", 1, 'test-champion-moves.js'],
  ['M158', "Make It Rain's Champions text says 1 stage again",
    "Sp. Atk by 2. Hits foe(s).", "Sp. Atk by 1. Hits foe(s).", 1, 'test-champion-moves.js'],
  ['M159', "The hover tooltip shows Scarlet/Violet's description in Champions",
  ...
  ['M73', 'The ability page stops adding species that had the ability then (Levitate loses Gengar)',
    /* Re-anchored in 5.50: the reverse scan became an index built once per generation, and this is
       the line that fills it. Emptying it is the same defect the old mutation made. */
    'rows.forEach(function(r){if(r.ability)(idx[r.ability]=idx[r.ability]||[]).push({species:species,row:r})});',
    'rows.forEach(function(r){});', 1, 'test-past-abilities.js'],

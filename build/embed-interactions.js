#!/usr/bin/env node
/* HoopaDex — put the derived interaction table into the app
 * Run: node build/generate-interactions.js && node build/embed-interactions.js
 *
 * Rewrites the region between /*BEGIN-INTERACTIONS* / and /*END-INTERACTIONS* / in app/index.html
 ...
 * diff at all. tests/test-interactions.js asserts the two are identical, so a stale embed fails the
...
const DATA = path.join(ROOT, 'data', 'interactions.embed.json');

const OPEN = '/*BEGIN-INTERACTIONS*/';
const CLOSE = '/*END-INTERACTIONS*/';

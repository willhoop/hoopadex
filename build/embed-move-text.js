#!/usr/bin/env node
/* HoopaDex — put the per-generation move text into the app
 * Run: node build/generate-move-text.js && node build/embed-move-text.js
 *
 * Rewrites the region between /*BEGIN-MOVETEXT* / and /*END-MOVETEXT* / in app/index.html
 ...
 * diff at all. tests/test-move-text.js asserts the two are identical, so a stale embed fails the
...
const DATA = path.join(ROOT, 'data', 'move-text.embed.json');

const OPEN = '/*BEGIN-MOVETEXT*/';
const CLOSE = '/*END-MOVETEXT*/';

...
## 4.5 Who has an ability: one resolver, two callers

**`abilityHoldersFiltered(abilityName, apiList, genNum, genMax)`** is the only supported answer to
"which Pokémon have this ability, in the game and generation currently selected". It applies four
gates, in order:

1. `abilityHoldersForGen(abilityName, apiList, genNum)` — resolves the holders **for that
   generation**, adding species that had the ability then and removing ones that gained it later.
   See `PASTABIL` and `build/generate-past-abilities.js`.
2. An unresolvable species id is dropped.
3. `formAllowed(name, id, genNum, genMax)` — carries the Champions roster (`CHAMPIONS_IDS`), the
   battle-only-form rule, the required-stone rule and the form-era range. Shared with the team
   builder search and the calculator picker.
4. Hidden-only holders are dropped below Generation V, where hidden abilities did not exist.

Both callers — `showAbilityPage` and `renderAbilityList` — use it. The list resolves the count
**once per ability** into a `Map` and both filters on it and prints it; resolving twice would double
the work across 373 abilities.

**Do not count holders any other way.** Until 5.41 the list used `id<=genMax` inline, which is gate
3 with everything interesting removed, and the two views disagreed on 267 of 373 rows in Champions
(and on 131 in Generation IX, in the opposite direction). `tests/test-ability-counts.js` asserts
structurally that the shortcut is absent from the list renderer; mutations M87–M93 in
`build/mutation-check.js` each reinstate one gate and must fail that suite.

**`speciesIdBySlug(slug)`** exists because `master` stores a handful of species under their default
form (`zygarde-50`, `pyroar-male`, `meowstic-male`), so an exact lookup on the bare species slug
misses them, and `baseSpeciesId` — which walks prefixes downward — cannot reach them. Exact match
wins over prefix match, so a hyphenated species name (`ho-oh`) is never mistaken for a form. An
unresolved slug returns `0`, and callers must treat that as "not in this game" rather than as
"unknown, allow it".

## 4.6 The Z-A mega table is not the Champions mega table

`CHAMP_MEGA_ABILITIES` is generated by `build/generate-mega-abilities.js` from Showdown's
`pokedex.ts`, using the app's own `MEGAS_ZA` set as the registry of what counts as official. That
set is **Pokémon Legends: Z-A**. It is not the Champions roster, and the two are not
interchangeable: eight of the 42 Z-A Megas are species `CHAMPIONS_IDS` does not contain
(Baxcalibur, Darkrai, Golisopod, Heatran, Magearna, Tatsugiri, Zeraora, Zygarde).

The `MEGA` badge on the abilities list is therefore filtered by the roster before it is drawn. The
